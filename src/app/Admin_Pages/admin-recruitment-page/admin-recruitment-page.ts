...
@Component({
  ...
  templateUrl: './admin-recruitment-page.html',
  styleUrl: './admin-recruitment-page.css',
})
export class AdminRecruitmentPage implements OnInit {
  ...
  closedCount = computed(() => this.allJobs().filter(j => j.status === 'Closed').length);
  ...
  openStatusModal(job: any, action: 'publish' | 'onhold' | 'close' | 'resume') {
    this.statusModal.set({ job, action });
    document.body.style.overflow = 'hidden';
  }
  ...
  confirmStatusChange() {
    ...
    const call$ =
      m.action === 'publish' ? this.recruitService.publishJob(jobId)
        : m.action === 'resume' ? this.recruitService.openJob(jobId)
          : m.action === 'onhold' ? this.recruitService.onHoldJob(jobId)
            : this.recruitService.closeJob(jobId);

    const msg: Record<string, string> = { publish: 'published', resume: 'Opened', onhold: 'put on hold', close: 'closed' };
    call$.subscribe({
  ...
  confirmDeleteJob(job: any) {
    this.deleteModal.set({ id: job.jobId, name: job.title });
    document.body.style.overflow = 'hidden';
  }
        ...
        this.candidatesMap.update(map => {
          ...
          for (const jobId in map) {
            updated[+jobId] = map[+jobId].map(c =>
              c.applicationId === applicationId ? { ...c, applicationStatus: stage } : c
            );
          }
          return updated;
        });
  ...
  canResume(s: string) { return s === 'OnHold' || s === 'Closed'; }
