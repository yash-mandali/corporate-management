...
export class ManagerLeavepage implements OnInit {
  ...
  // ── Computed stats ──
  pendingCount = computed(() => this.allLeaves().filter(l => l.status === 'Pending').length);
  approvedCount = computed(() => this.allLeaves().filter(l => l.status === 'Approved').length);
  rejectedCount = computed(() => this.allLeaves().filter(l => l.status === 'Rejected').length);
  withdrawnCount = computed(() => this.allLeaves().filter(l => l.status === 'Withdrawn').length);
  ...
  filteredHistory = computed(() => {
    ...
    return this.allLeaves()
      .filter(l => {
        ...
        const matchSt = sf === 'all' || l.status === sf;
