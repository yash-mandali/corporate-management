...
export class PayrollRecruitmentPage implements OnInit {
  ...
  currentUserId = signal<number>(16); // TODO: get from AuthService
  Math = Math;
  ...
  // Applied job IDs set — built from myApplications
  appliedJobIds = computed(() => new Set(this.myApplications().map((a: any) => a.jobId ?? a.candidateId)));
  ...
  // ── Load my applications: fetch candidates for all jobs, filter by userId ──
  loadMyApplications() {
        ...
        visibleJobs.forEach(job => {
          this.recruitService.getCandidatesByJobId(job.jobId).subscribe({
            next: (cRes: any) => {
              ...
              const mine = candidates.filter((c: any) => c.userId === this.currentUserId());
